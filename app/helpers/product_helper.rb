...
module ProductHelper
  ...
  def product_color_presence_link(product)
    unless params[:subcategory_id].nil?
      color_presence_admin_subcategory_product_path(params[:subcategory_id], product)
    else
      return params[:brand_id].nil? ? color_presence_admin_product_path(product) : color_presence_admin_brand_product_path(params[:brand_id], product)
    end
  end
